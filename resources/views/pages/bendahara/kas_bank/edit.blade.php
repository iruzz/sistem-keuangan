...
@section('page-title', 'Edit Kas/Bank')
...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-md border border-gray-100">
    <h2 class="text-2xl font-bold mb-6 text-gray-800">Edit Kas/Bank</h2>
    ...
    <form action="{{ route('bendahara.kasBank.update', $kasBank->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Rekening</label>
            <input type="text" name="nama_rekening" value="{{ $kasBank->nama_rekening }}"
        ...
        <div>
            ...
            <input type="text" name="nomor_rekening" value="{{ $kasBank->nomor_rekening }}"
                class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
        </div>
        ...
        <div>
            ...
            <select name="tipe" class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 outline-none transition" required>
                <option value="Kas" {{ $kasBank->tipe === 'Kas' ? 'selected' : '' }}>Kas</option>
                <option value="Bank" {{ $kasBank->tipe === 'Bank' ? 'selected' : '' }}>Bank</option>
            </select>
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Saldo</label>
            <input type="number" name="saldo" value="{{ $kasBank->saldo }}"
                class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
                min="0">
        </div>
